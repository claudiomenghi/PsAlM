...
public enum CoreMovementPatterns {
	...
	VISIT {
		...
		public Pattern getMission(String[] locations) {
			return new Visit(
					Arrays.asList(locations).stream().map(s -> new PLAtom("robot", s)).collect(Collectors.toList()));
	...
	SEQUENCED_PATROLLING {
		...
		@Override
		public Pattern getMission(String[] locations) {
			return new SequencedPatrolling(
					Arrays.asList(locations).stream().map(s -> new PLAtom("robot", s)).collect(Collectors.toList()));
		}
	...
	STRICT_ORDERED_PATROLLING {
		...
		@Override
		public Pattern getMission(String[] locations) {
			return new StrictOrderedPatrolling(
					Arrays.asList(locations).stream().map(s -> new PLAtom("robot", s)).collect(Collectors.toList()));
		}
	...
	public abstract Pattern getMission(String[] locations);
